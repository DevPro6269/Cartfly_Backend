...
const skip = req.params.skip||0
  ...
  const prodcuts = await Product.find().limit(limit).skip(skip)
  if(!prodcuts) return res.status(404).json(new ApiError(404,"products not found"));

  return res.status(200).json(new ApiRespone(200,prodcuts,"success"))
}